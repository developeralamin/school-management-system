		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			  <h4 class="box-title">Update Employee Attendance</h4>
...
<div class="row">
	...
	<div class="form-group">
		...
		<div class="controls">
		 <input type="date" name="date" id="date" value="{{ $editData['0']['date'] }}" required="" class="form-control">
  ...
  <div class="row">
	...
	<div class="switch-toggle switch-3 switch-candy">
		...
		<input type="radio" name="attendace_status{{$key}}" id="present{{$key}}"
		 value="Present" checked="checked" {{ ($data->attendace_status == 'Present')?'checked':'' }}>
		<label for="present{{$key}}">Present</label>

		<input type="radio" name="attendace_status{{$key}}" value="Leave" {{ ($data->attendace_status == 'Leave')?'checked':'' }} id="leave{{$key}}">
		<label for="leave{{$key}}">Leave</label>

		<input type="radio" name="attendace_status{{$key}}" value="Absent" {{ ($data->attendace_status == 'Absent')?'checked':'' }} id="absent{{$key}}">
